...
exp_variance$hypotheses$choices_value_low_variance <- conditional_hypothesis(
  exp_variance$models$choices, 
  hypothesis = "High value > Low value", 
  effect_variables = "trial_type", 
  conditional_variables = "condition",
  conditional_levels = "Low variance"
)
...
exp_variance$hypotheses$ftm_value_low_variance <- conditional_hypothesis(
  exp_variance$models$ftm, 
  hypothesis = "High value > Low value", 
  effect_variables = "stimulus", 
  conditional_variables = "condition",
...
exp_variance$hypotheses$percent_value_low_variance <- conditional_hypothesis(
  exp_variance$models$percent, 
  hypothesis = "High value > Low value", 
  effect_variables = "stimulus", 
  conditional_variables = "condition",
...
exp_skew_edges$hypotheses$choices_value_interaction <- conditional_hypothesis(
  exp_skew_edges$models$choices_value, 
  hypothesis = "High value:Experiment a < High value:Experiment b", 
  effect_variables = c("option_value", "experiment"),
  conditional_variables = "none"
)
...
## Test hypotheses based on model
exp_skew_tokens$hypotheses$choices_value_interaction <- conditional_hypothesis(
  exp_skew_tokens$models$choices_value, 
  hypothesis = "option_value:Right-skewed > option_value:Left-skewed", 
  effect_variables = c("option_value", "condition"), 
  conditional_variables = "none"
)


## Test hypotheses based on model
exp_skew_tokens$hypotheses$choices_value_left_skewed <- conditional_hypothesis(
  exp_skew_tokens$models$choices_value, 
  hypothesis = "option_value > 0", 
  effect_variables = "option_value", 
  conditional_variables = "condition",
  conditional_levels = "Left-skewed"
)

# Compare the difference between the medium-value option and the high-value/low-value options
exp_skew_tokens$models$choices_value_comparison <- brm(
  risky_count | trials(n_choices) ~ 1 + option_value_unordered * condition + (1 + option_value_unordered | participant),
  data = exp_skew_tokens$choices_summary %>% filter(!participant %in% exp_skew_tokens$exclusions, trial_type != "Catch") %>% mutate(option_value_unordered = option_value %>% factor(ordered = FALSE) %>% named_contr_sum(., return_contr = FALSE)),
  family = binomial,
  prior = c(priors$basic, priors$varying_intercepts, priors$varying_slopes),
  warmup = ifelse(getOption("quick_version"), 1000, 2000),
  iter = ifelse(getOption("quick_version"), 4000, floor(150000 / chains) + 2000),
  file = save_model("choices_value_comparison", "exp_skew_tokens"),
  thin = ifelse(getOption("quick_version"), 1, 5),
  cores = cores,
  chains = chains,
  backend = "cmdstanr"
...
## Test hypotheses based on model
exp_skew_tokens$hypotheses$choices_value_high <- hypothesis(
  exp_skew_tokens$models$choices_value_comparison,
  "option_value_unorderedHighvalue > 0"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")

## Test hypotheses based on model
exp_skew_tokens$hypotheses$choices_value_low <- hypothesis(
  exp_skew_tokens$models$choices_value_comparison,
  "option_value_unorderedLowvalue > 0"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")

exp_skew_tokens$hypotheses$choices_value_difference <- hypothesis(
  exp_skew_tokens$models$choices_value_comparison,
  "abs(option_value_unorderedHighvalue) > abs(option_value_unorderedLowvalue)"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")
...
# Compare the difference between the medium-value option and the high-value/low-value options
exp_skew_tokens$models$ftm_value_comparison <- brm(
  response_recode_value  ~ 1 + stimulus_value_unordered * condition + (1 + stimulus_value_unordered || participant),
  data = exp_skew_tokens$ftm_recode %>% filter(!participant %in% exp_skew_tokens$exclusions) %>% mutate(stimulus_value_unordered = stimulus_value %>% factor(ordered = FALSE) %>% named_contr_sum(., return_contr = FALSE)),
  family = categorical(refcat = "Worse"),
  prior = c(priors$basic_ftm, priors$varying_intercepts_ftm),
  warmup = ifelse(getOption("quick_version"), 1000, 2000),
  iter = ifelse(getOption("quick_version"), 2000, floor(20000 / chains) + 2000),
  file = save_model("ftm_value_comparison", "exp_skew_tokens"),
  control = list(adapt_delta = 0.9),
  cores = cores,
  chains = chains,
  backend = "cmdstanr"
...
## Test hypotheses based on model
exp_skew_tokens$hypotheses$ftm_value_high <- hypothesis(
  exp_skew_tokens$models$ftm_value_comparison,
  "muBetter_stimulus_value_unorderedHighvalue > 0"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")

## Test hypotheses based on model
exp_skew_tokens$hypotheses$ftm_value_low <- hypothesis(
  exp_skew_tokens$models$ftm_value_comparison,
  "muBetter_stimulus_value_unorderedLowvalue > 0"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")

exp_skew_tokens$hypotheses$ftm_value_difference <- hypothesis(
  exp_skew_tokens$models$ftm_value_comparison,
  "abs(muBetter_stimulus_value_unorderedHighvalue) > abs(muBetter_stimulus_value_unorderedLowvalue)"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")
...
# Compare the difference between the medium-value option and the high-value/low-value options
exp_skew_tokens$models$percent_value_comparison <- brm(
  bf(
    # Varying slope removed from maximal model for convergence issues (divergences and low ESS for SD)
    # Nonetheless, these models produced very similar estimates for the parameters of interest
    better_minus_worse ~ 1 + stimulus_value_unordered * condition + (1 | participant),
    sigma ~ 1 + stimulus_value_unordered * condition
  ),
  data = exp_skew_tokens$percent_recode %>% 
    filter(!participant %in% exp_skew_tokens$exclusions) %>% 
    mutate(better_minus_worse = standardise(better_minus_worse)) %>% 
    mutate(stimulus_value_unordered = stimulus_value %>% factor(ordered = FALSE) %>% named_contr_sum(., return_contr = FALSE)),
  prior = c(
    priors$basic, 
    priors$varying_intercepts,
    priors$basic_sigma
  ...
  warmup = ifelse(getOption("quick_version"), 1000, 2000),
  iter = ifelse(getOption("quick_version"), 2000, floor(20000 / chains) + 2000),
  file = save_model("percent_value_comparison", "exp_skew_tokens"),
  control = list(adapt_delta = 0.99),
  cores = cores,
  chains = chains,
  backend = "cmdstanr"
...
## Test hypotheses based on model
exp_skew_tokens$hypotheses$percent_value_high <- hypothesis(
  exp_skew_tokens$models$percent_value_comparison,
  "stimulus_value_unorderedHighvalue > 0"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")

## Test hypotheses based on model
exp_skew_tokens$hypotheses$percent_value_low <- hypothesis(
  exp_skew_tokens$models$percent_value_comparison,
  "stimulus_value_unorderedLowvalue > 0"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")

exp_skew_tokens$hypotheses$percent_value_difference <- hypothesis(
  exp_skew_tokens$models$percent_value_comparison,
  "abs(stimulus_value_unorderedHighvalue) > abs(stimulus_value_unorderedLowvalue)"
) %>%
  summarise_hypothesis_output("median", 0.95, "hdi")
